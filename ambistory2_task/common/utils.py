...
def print_annotation_schema(subtask: str, index: int) -> tuple:
    ...
    if subtask == "qualification":
        samples = read_json_from_file(TASK_INFO["ambistory2_task"]["qualification_filepath"])
    else:
        samples = read_json_from_file(TASK_INFO["ambistory2_task"]["annotation_filepath"])
    ...
    if "precontext" in question:
        precontext = question["precontext"].replace("\n", "")
        sentence = question["sentence"].replace(question["word"], "[" + question["word"] + "]")

        st.markdown(format_sentence(precontext + "\n" + sentence))
    else:
        st.markdown(question["sentence"].replace("[" + question["word"] + "]", ":blue-background[" + question["word"] + "]"))
