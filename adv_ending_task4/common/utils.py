...
with open("adv_ending_task4/resources/tutorial_text.json", "r") as f:
...
def print_annotation_schema(subtask: str, index: int) -> tuple:
    ...
    if subtask == "qualification":
        samples = read_json_from_file(TASK_INFO["adv_ending_task4"]["qualification_filepath"])
        ...
        if subtask == "annotation":
            ...
            samples = read_json_from_file(TASK_INFO["adv_ending_task4"]["tutorial_filepath"])
...
- Select story parts from the templates to create a context in which the above meaning is slightly more plausible for the given ambiguous words.
...
- Edit templates as much as you need to to imply one of the meanings, but also e.g. to fix logical inconsistencies.
                            
- You must edit either the first or second part.
                            
- You must not use the ambiguous word in your context.
