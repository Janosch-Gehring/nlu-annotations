...
def skip_to_next_sample(index: int, samples: dict, grouping: int, direction: int=1, 
    ...
    grouping = (grouping + 2 * loops_done) % TASK_INFO["adv_ending_task3"]["number_of_annotator_groups"] 
    ...
    while True:
        if index > len(samples):
            ...
            if grouping == start_grouping:
                ...
                grouping = (grouping + 2) % TASK_INFO["adv_ending_task3"]["number_of_annotator_groups"] 
...
st.session_state.page = "adv_ending_task3_annotation_page_sample" + str(st.session_state.progress)
